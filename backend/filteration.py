...
destination = pd.read_csv('C:\\IIT\\roam_routely\\dataset\\dataset.csv')
...
def generate_itinerary(_id):
    ...
    # Retrieve the last document sorted by createdAt field
...
def extract_distance(distance_str):
    if isinstance(distance_str, tuple):  
        distance_str = distance_str[0]  # Take the first element of the tuple
    # Extract the numerical value
    distance_value = float(distance_str.split()[0])
    return distance_value


# Function to sort locations based on distance
